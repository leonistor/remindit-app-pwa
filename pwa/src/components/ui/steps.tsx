...
export const StepsIndicator = (
  ...
  return (
    <ArkSteps.Indicator
      ...
      {...rest}
    >
      {/* Key the number→check swap off the indicator's OWN data-complete:
          Ark's Item part only gets `aria-current` (no `data-complete`), so
          `group-data-complete/step:` variants would never match. */}
      <span className="data-complete:hidden">{children}</span>
      <Check className="hidden data-complete:block" />
    </ArkSteps.Indicator>
  )
}
