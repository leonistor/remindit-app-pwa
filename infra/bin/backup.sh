#!/usr/bin/env sh
# Scheduled backup entrypoint for the systemd timer: snapshots PocketBase's
# pb_data/ via the superuser API, then mirrors the local backup dirs off-box
# to Scaleway S3 via rclone (D11). Loads the repo-root .env via bun
# --env-file (dotenv semantics — no shell sourcing; SCW_* values are parsed
# by grep, they never contain spaces) and self-locates the repo root.
#
# Tolerance ladder: the PB half is critical (failure aborts the run); the
# off-box copy is best-effort again — local archives always remain the source
# of truth. rclone copy never deletes remotely; off-box retention is its own
# window (--min-age), so a local wipe cannot take the off-box copies down
# with it.
set -e
...
then
  ...
  echo "[backup] off-box copy done (s3.$SCW_REGION.scw.cloud/$SCW_BUCKET/pb/)"
else
