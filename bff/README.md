...
| `POST /api/ai/chat` | AI support chat (**optional-auth**, D15): streams a VoltAgent support-assistant reply (grounded on `bff/content/support-en.md`, answers in the user's app locale) as a UIMessageStreamResponse for the pwa's Assistant UI. Authed users get **persistent single-thread memory** (server-owned `conversations` collection via VoltAgent's StorageAdapter, keyed by `userId`/`conversationId` — the client's thread `id`); anonymous callers fall back to in-memory. When the body carries the pwa's active `teamId`, the agent gains the **app-command tools** (Task B, D15): `list_items`/`recommend_items` are server-executed (read the user's team via `bff/src/services/ai-context.ts` + `@remindit/common/recommender`); `add_item` is a server-confirm, client-executed write (the pwa runs `createItemAndAddToList` locally) |
| `GET /api/ai/context?teamId=` | **Task B (authed):** the signed-in user's team context for the commands — current list + catalog + categories + recommendations (`computeRecommendations` from `@remindit/common/recommender`). 401 without a token |
