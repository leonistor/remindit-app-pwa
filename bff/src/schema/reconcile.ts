// Reconcile the live PocketBase schema towards `collections.ts` (D7).
//
// The migrate script (scripts/migrate.ts) is a thin entry over this module so
// the engine is unit-testable without a live PB: `reconcileSchema(pb)` runs
// the full create/patch/verify passes against any client and returns the
// human-readable action log. Never deletes: per collection — create if
// missing, patch if the canonicalized definition drifted, skip when
// unchanged. Running twice in a row must report everything "unchanged" (the
...
/**
 * Reconcile the live schema towards {@link desiredCollections}.
 *
 * @param pb   an authenticated (superuser) PB client
 * @param opts.promoteAdminEmail  if set, promote the matching registered user
 *   to `admin` role (phase 6 role bootstrap) — idempotent.
 * @returns the human-readable action log (one line per collection).
 */
export async function reconcileSchema(
  pb: PocketBase,
  opts: { promoteAdminEmail?: string } = {}
): Promise<string[]> {
