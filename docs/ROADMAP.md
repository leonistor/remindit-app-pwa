...
- [x] **Basic AI features — shipped 2026-09-08** — app "commands" (D15): `list`/`recommend`/`add` via client-executed tools (server-confirm `add`, pwa-local write → sync), `GET /api/ai/context`, and the persistent single-thread memory slice. Remaining (out of scope): full write set (remove/clear)
