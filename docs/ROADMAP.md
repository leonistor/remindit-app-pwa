...
- [x] **AI phase 2 slices (2026-09-08)** — **feedback capture** (write-only `feedback` collection + optional-auth `POST /api/feedback`, dedicated composer in the assistant view — restores the in-app feedback channel removed in D13) and **conversation language from user profile** (assistant answers in the user's app locale; per-locale agent cache).
- [ ] Basic AI features — **phase 2+ remaining**: app "commands" (show list, recommendations, `add "mustard"` — needs auth + tool-calling + a write path decision); persistent multi-thread memory
