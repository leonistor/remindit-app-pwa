...
- [x] **AI support chat (phase 1, 2026-09-07, merged 2026-09-08)** — embedded VoltAgent support assistant in the pwa (`/assistant`), grounded on `bff/content/support-en.md`, with a model eval harness (`bun run model:eval`). Models chosen by eval: local `qwen2.5:7b`, remote `minimax/minimax-m3:free` (inkling/nemotron-super dropped — agentic-only / Invalid-JSON). Remote default switched 2026-09-08 to `nvidia/nemotron-3-super-120b-a12b:free` after the provider retired minimax-m3:free (paid-only). Evaluated and kept.
- [x] **AI phase 2 slices (2026-09-08)** — **feedback capture** (write-only `feedback` collection + optional-auth `POST /api/feedback`, dedicated composer in the assistant view — restores the in-app feedback channel removed in D13), **conversation language from user profile** (assistant answers in the user's app locale; per-locale agent cache), and **persistent single-thread memory** (D15 — `conversations` collection + VoltAgent StorageAdapter; `POST /api/ai/chat` optional-auth, authed users PB-backed, anonymous in-memory).
- [ ] Basic AI features — **phase 2+ remaining**: app "commands" (show list, recommendations, `add "mustard"` — needs tool-calling + a write path; D15: tools run client-side, slice = read-only + `add`). Persistent single-thread memory **shipped 2026-09-08** (D15, `conversations` collection + optional-auth chat)
