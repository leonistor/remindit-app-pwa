...
`.env.example` values, and `feedback` wired into `dev:all`.
...
existing-collection field drift BEFORE creating views (PB's fresh-install
default `users` has no `username` column and a file-typed avatar — fresh
install + idempotency verified against a scratch PB); `infra/bin/bootstrap-prod.sh`
is the one-time privileged bootstrap (backup timer, Caddy + admin hash,
`bm2 save`/`startup install`); deploy flow = `git push vps main` (server repo
at `/srv/remindit`, `receive.denyCurrentBranch=updateInstead`).

- [x] **D1** VPS: `bun add -g bm2`, `bm2 start infra/ecosystem.config.ts` →
  ...
  **Back up the feedback module's `answer-data/` alongside `pb_data/`** (same
  ...
  blocks `pb_data/`). Local archive + restore round-trip verified. VPS side:
  bm2 5/5 online + hourly `remindit-backup.timer` enabled via
  `bootstrap-prod.sh`; the explicit reboot check was skipped by decision
  (2026-09-04) — `bm2 save` + `startup install` ran clean; persistence gets
  proven at the next natural reboot. Backups verified live the same day after
  one more fix: the systemd unit now runs as leo with `~/.bun/bin` on PATH
  (root's default PATH broke both halves with "bun: not found") — a manual
  `remindit-backup.service` run produced both a `remindit-<ts>.zip` and an
  `answer-<ts>.tar.gz`.
- [x] **D2** Build + deploy `web/` + `admin/` SSR behind Caddy
  (`www.remindit.me` / `admin.remindit.me`); protect admin origin (`basic_auth`
  hash generated by `bootstrap-prod.sh`, optional IP allowlist still commented
  in `infra/Caddyfile`). Verified live: 200 / 401-anon, TLS via ACME. DNS A
  records for the subdomains went in first (Caddy needs them for cert
  issuance — decided 2026-09-04); the apex was already pointing at the VPS.
- [x] **D3** Prod env plumbing on the VPS repo-root `.env`:
  ...
  / `PUBLIC_FEEDBACK_URL=https://feedback.remindit.me`. Done 2026-09-04:
  written on the VPS with `openssl`-generated secrets (PB superuser, Answer
  admin, `ANSWER_BRIDGE_SECRET`), chmod 600; CORS preflight verified from the
  real origin; SMTP points at the Inbucket on the same box (dev-grade — swap
  via `configure:feedback smtp` when a real provider is needed).
- [x] **D4** Deploy current pwa bundle to `remindit.me` (SW-safe release flow,
  `pwa/docs/DEPLOY.md`); DNS cutover from `remindit.parsedwink.com`. Done
  2026-09-04: prod zip built locally with real `PUBLIC_*` (localhost refs
  DCE'd — verified in the served bundle), extracted to `/var/www/remindit`,
  apex live. The old static host `remindit.parsedwink.com` still serves the
  previous bundle from `/var/web/remindit` — leave or 301 at the Caddy level
  when ready (no user data there; the PWA installs live client-side).
- [x] **D5** Deploy the Feedback module (Apache Answer) as part of the platform
  rollout: bm2-managed `feedback` process + Caddy host `feedback.remindit.me`.
  Done 2026-09-04: tags/branding/smtp configured on prod; guest submit flow
  e2e-verified live (BFF → `web-guest` twin → Answer question URL returned).
  FB2–FB6 shipped 2026-09-04; remaining niceties: manual logo upload in the
  Answer admin, real SMTP provider, delete the smoke-test question.
