// Recommendation engine — pure-function scoring that derives item
// recommendations from shopping history, category frequency defaults, and
// catalog data.
//
// Lives in `@remindit/common` (not the pwa) so the BFF can run the same
// scoring for the AI "recommend" command and the context route (Task B):
// it is a pure computation over plain data with zero store or UI
// dependencies — every consumer passes in history/catalog/categories/list as
// plain arrays. The pwa re-exports it via `pwa/src/stores/recommender.ts`.

import { FREQ_TO_DAYS } from "./seeds/history"
import type {
  CatalogItem,
  Category,
  CategoryFrequency,
...
import { UNCATEGORIZED_NAME } from "./models/types"
